...
** emacs init filer
*** early-init.el (require emacs version > 27.0)
#+begin_example
...
- [[#preface][Preface]]
  ...
  - [[#emacs-init-filer][emacs init filer]]
    - [[#early-initel-require-emacs-version--270][early-init.el (require emacs version > 27.0)]]
    - [[#initel][init.el]]
- [[#basic-emacs][Basic emacs]]
  - [[#fundamental-constants-and-functions][Fundamental constants and functions]]
    - [[#better-defaults][Better defaults]]
    - [[#common-and-helpers][Common and Helpers]]
    - [[#version-control-and-warning-if-wrong-version][version control and warning if wrong version]]
  ...
  - [[#buffer][Buffer]]
    ...
    - [[#insert-char-preview][insert-char-preview]]
    ...
    - [[#fill-a-paragraph][fill a paragraph]]
  ...
  - [[#misc][Misc]]
    ...
    - [[#copy-id][copy ID]]
    - [[#running-on-a-macos][running on a macOS]]
...
:END:
I've tried to make this in an orderdly manner, which is not an 
easy task for me, as the is far from my normally behavior.
...
*** Common and Helpers
:PROPERTIES:
:ID:       7B041D81-6330-43C6-95A6-ED94E7C46D3F
:END:
The general package is a helper package for defing key bindings and settings i a
unified way. 
se https://github.com/noctuid/general.el for documentation
#+begin_src emacs-lisp :tangle yes
(straight-use-package '(general :type git :flavor melpa :host github :repo "noctuid/general.el"))
...
#+end_src
;;
Beneath are several macros, to help define things
...
(if nec/measure-time (nec/timer "Common and Helpers "))
#+end_src
*** version control and warning if wrong version
:PROPERTIES:
...
what is stored in that added =custom= snippet. But......
#+begin_src emacs-lisp
...
(if nec/measure-time (nec/timer "Load custom.el   "))
#+end_src
*** commify
...
single command. This is an otherwise tedious chore and one that is not easily
done via a macro or other Emacs user-level facilities.
In our contry we are using "," for decimal and "." for tousand divider
#+begin_src emacs-lisp :tangle yes
(straight-use-package '(key-chord :type git :flavor melpa :host github 
                                  :repo "emacsorphanage/key-chord"))
(straight-use-package '(commify :type git :flavor melpa :host github 
...
(straight-use-package '(bookmark+ :type git :host github 
...
(setenv "PATH" "/bin:/usr/bin:/usr/local/bin:/sbin:/usr/sbin:/usr/local/opt:/usr/local/bin:/usr/local/sbin:/Library/Tex/texbin:/Users/niels/.cargo/bin:/opt/X11/bin")
...
;; Download icons
 ...
 (straight-use-package '(all-the-icons :type git :flavor melpa 
...
(straight-use-package '(all-the-icons-ibuffer :type git :flavor melpa 
                 :host github :repo "seagle0128/all-the-icons-ibuffer"))
...
(straight-use-package '(org-pretty-tags :type git :flavor melpa 
                                        :host gitlab :repo "marcowahl/org-pretty-tags"))
   (setq org-pretty-tags-mode-lighter " 📤")
   (setq org-pretty-tags-surrogate-strings
         '(("log"     . "✍")
           ...
           ("bil"      . "🚗") ; 🚙 🚗 🚘
           ("haster"   . "🔥")
           ("vigtigt" . "☆") ; important stuff.
           ("ide" . "💡") ; inspiration.
           ("penge" . "$$$")
           ("let" . "₰")
           ("musik" . "♬"))) ;; 📥 📤 📬
   (setq org-pretty-tags-global-mode 1)
(if nec/measure-time (nec/timer "Variables" ))
#+end_src
*** emacs-svg-icon
#+begin_src emacs-lisp
(require 'svg-icon)
...
(straight-use-package '(doom-themes :type git :flavor melpa :files (:defaults "themes/*.el" "doom-themes-pkg.el") :host github :repo "hlissner/emacs-doom-themes"))
#+end_src
...
#+end_src
*** insert-char-preview
Insert Unicode char via insert-char-preview similar to insert-char command, but
with character preview in completion prompt. from: [[https://gitlab.com/matsievskiysv/insert-char-preview/tree/0e4a62b5407fb1bed8920a4c13cf9a91065e15ad][insert-char-preview]]
#+begin_src emacs-lisp :tangle yes
(straight-use-package '(insert-char-preview :type git :flavor melpa :host gitlab :repo "matsievskiysv/insert-char-preview"))
;;  :commands insert-char-preview
;;  :bind ("C-x 8 RET" . insert-char-preview))
(if nec/measure-time (nec/timer "insert-char-preview "))
#+end_src
*** scratch buffer
#+begin_src emacs-lisp :tangle yes
;; Makes *scratch* empty.
...
(if nec/measure-time (nec/timer "Create missing files   "))
#+end_src
*** Create tmp directory
#+begin_src emacs-lisp
(setq nec/tmp-dir "~/.emacs.d/private/tmp")
(when (file-exists-p nec/tmp-dir)
      (delete-directory nec/tmp-dir t))
(make-directory nec/tmp-dir t)
(if nec/measure-time (nec/timer "reset tmp directory"))
#+end_src
*** kill buffer and the file associated
 #+BEGIN_SRC emacs-lisp :tangle yes
     (defun bk/kill-buffer-and-file (buffer-name)
       "Removes file connected to current buffer and kills buffer."
       (interactive "Kill buffer and its file:")
...
#+begin_src emacs-lisp
(use-package midnight :straight t)
...
(gdefine-key "C-x C-s" #'save-all-buffers)
...
(use-package disk-usage
  :straight t
  :commands (disk-usage))
(if nec/measure-time (nec/timer "Disk usage   "))
#+end_src
*** hostname
:PROPERTIES:
:CUSTOM_ID: hostname
...
#+begin_src emacs-lisp :tangle yes
;;; Code:
(straight-use-package '(shrink-path :type git :flavor melpa 
                        :host gitlab :repo "bennya/shrink-path.el"))
#+end_src
...
:END:
#+BEGIN_SRC emacs-lisp
(straight-use-package '(ace-window :type git :flavor melpa :host github :repo "abo-abo/ace-window"))

(straight-use-package '(ace-jump-mode :type git :flavor melpa :host github :repo "winterTTr/ace-jump-mode"))
...
;;  (helm-mode 1)
(straight-use-package '(helm-bibtex :type git :flavor melpa 
                        :files ("helm-bibtex.el" "helm-bibtex-pkg.el") :host github :repo "tmalsburg/helm-bibtex"))
(straight-use-package-lazy '(helm-ag :type git :flavor melpa :host github :repo "emacsorphanage/helm-ag"))
(straight-use-package '(helm-org :type git :flavor melpa :files ("helm-org.el" "helm-org-pkg.el") :host github :repo "emacs-helm/helm-org"))
...
*** ivy
:PROPERTIES:
:ID:       0980AA2F-D1C5-4C28-AB7A-D34700A62728
:END:
...
#+begin_src emacs-lisp
(straight-use-package '(marginalia :type git :flavor melpa :host github 
                       :repo "minad/marginalia"))
(global-set-key (kbd "C-M-a") 'merginalia-cycle)
#+end_src
*** SMEX
:PROPERTIES:
:CUSTOM_ID: SMEX
...
#+begin_src emacs-lisp
(straight-use-package '(smartparens :type git :flavor melpa :host github :repo "Fuco1/smartparens"))
...
(straight-use-package '(company-lsp :type git :flavor melpa :host github 
...
;; bibtex
(straight-use-package '(company-bibtex :type git :flavor melpa 
                         :host github :repo "gbgar/company-bibtex"))
...
(if nec/measure-time (nec/timer "Company-box & lsp  "))
#+end_src
*** hippie Expand
:PROPERTIES:
...
(gdefine-key "<f2>" 'open-dashboard)


(gdefine-key "<f5>" 'my/copy-id-to-clipboard)
(gdefine-key "<f6>" 'ivy-resume)
(gdefine-key "C-c g" 'counsel-git)
;; zoom in/out the screen
(gdefine-key "C-+" 'text-scale-increase)
(gdefine-key "C--" 'text-scale-decrease)
(gdefine-key "C-M-=" 'increase-default-font-height)
(gdefine-key "C-M--" 'decrease-default-font-height)
;; crux
(gdefine-key "C-a"  'crux-move-beginning-of-line)
(gdefine-key "C-x 4 t"  'crux-transpose-windows)
(gdefine-key "C-x K"  'crux-kill-other-buffers)
(gdefine-key "M-o"  'crux-smart-open-line)
(gdefine-key "C-c D"  'crux-delete-file-and-buffer)
(gdefine-key "C-c e i"  'crux-find-user-init-file)
(gdefine-key "C-k"  'crux-smart-kill-line)
(gdefine-key "C-c c b" 'crux-buffer-mode)
(gdefine-key "C-c c R" 'crux-rename-file-and-buffer)
(gdefine-key "C-c c D" 'crux-delete-file-and-buffer)

;; org roam
(gdefine-key "C-c r l"  'org-roam)
(gdefine-key "C-c r g"  'org-roam-graph)
(gdefine-key "C-c r f"  'org-roam-find-file)
(gdefine-key "C-c r i"  'org-roam-insert)
(gdefine-key "C-c r c"  'org-roam-capture)
(gdefine-key "C-c r x"  'org-roam-jump-to-index)
(gdefine-key 
  :keymaps 'org-mode-map
           "C-c r c" 'org-roam-capture
           "C-c r x" 'org-jump-to-index
           "C-c >" 'org-insert-structure-template) 
(gdefine-key "C-c r r" 'org-backlink-mode-refresh-rate)
(gdefine-key "C-c r s" 'org-backlink-mode-refresh-rate)  
(gdefine-key "C-c r m" 'org-backlink-mode)  
(gdefine-key "C-c s s"  'org-super-links-link)
(gdefine-key "C-c s l"  'org-super-links-store-link)
(gdefine-key "C-c s C-l"  'org-super-links-insert-link)
(gdefine-key "C-c s d"  'org-super-links-quick-insert-drawer-link)
(gdefine-key "C-c s i"  'org-super-links-quick-insert-inline-link)
(gdefine-key "C-c s C-d"  'org-super-links-delete-link)
;; ivy
(gdefine-key  "C-s" 'swiper)
(gdefine-key "C-c C-r" 'ivy-resume)

(gdefine-key "M-x" 'counsel-M-x)
(gdefine-key "C-x C-f" 'counsel-find-file)

(gdefine-key "C-c j" 'counsel-git-grep)
(gdefine-key "C-c k" 'counsel-ag)
(gdefine-key "C-x l" 'counsel-locate)
(gdefine-key "C-S-o" 'counsel-rhythmbox)

(gdefine-key 
  :keymaps 'minibuffer-local-map 
           "C-r" 'counsel-minibuffer-history)
;; magit
(gdefine-key "C-c g"  'magit-status)
(gdefine-key "C-x g x"  'magit-checkout)
(gdefine-key "C-x g c"  'magit-commit)
(gdefine-key "C-x g p"  'magit-push)
(gdefine-key "C-x g u"  'magit-pull)
(gdefine-key "C-x g e"  'magit-ediff-resolve)
(gdefine-key "C-x g r"  'magit-rebase-interactive)
;; misc
(gdefine-key "M-g M-g" 'hydra-git-gutter/body)
(gdefine-key "C-c f"  'elfeed-dashboard)
(gdefine-key "C-M-S-s-d" 'deft)
(gdefine-key "C-x C-g" 'deft-find-file)
(gdefine-key "C-c n e"  'ebib)
(gdefine-key "C-c n r"  'org-ebib-open)
(gdefine-key "C-c n s"  'org-ebib-store-link)
(gdefine-key "C-c d"  'docker)

(gdefine-key "C-c C-c C-b" 'cargo-build)
(gdefine-key "C-c C-c C-r" 'cargo-run)
(gdefine-key "C-c C-c C-t" 'cargo-test)
(gdefine-key "C-c q" 'auto-fill-mode)
(gdefine-key "C-c l" 'org-store-link)
(gdefine-key "C-c a" 'org-agenda)
(gdefine-key "C-c c" 'org-capture)
(gdefine-key 
  :keymaps 'projectile-mode-map 
    "C-c p" 'projectile-command-map)
(gdefine-key "C-a" 'beginning-of-line-dwim)
(gdefine-key "C-x o" 'ace-window)
(gdefine-key "C-&" 'gnus)
(if nec/measure-time
...
(straight-use-package '(info+ :type git :host github :repo "emacsmirror/info-plus"))
(straight-use-package '(ghub :type git :flavor melpa :host github :repo "magit/ghub"))

(straight-use-package '(doom-modeline :type git :flavor melpa :host github 
                        :repo "seagle0128/doom-modeline"))

;;;;;;;ns  issue #406 doom-modeline
(setq battery-status-function nil)
;;;;;;;ns
(setq doom-modeline-height 1.4)
;;(set-face-attribute 'mode-line nil :family "OpenDyslexic" :background "#2d334a" :height 125)
;;(set-face-attribute 'mode-line-inactive nil :family "OpenDyslexic" :height 125)
...
(if nec/measure-time (nec/timer "terminal "))
#+end_src
*** edit command
:PROPERTIES:
...
#+begin_src emacs-lisp
    (straight-use-package '(page-break-lines :type git :flavor melpa :host github
    ...
    (global-page-break-lines-mode)
      ...
      (setq dashboard-banner-logo-title "Open EMACS for a new world...")
      (setq dashboard-startup-banner "/Users/niels/.emacs.d/images/ohands.png")
      (setq dashboard-items '((bookmarks . 6) (agenda . 6)))
...
;;      (setq dashboard-banner-logo-title ((t (:family "Love LetterTW" :height 123)))))
        (straight-use-package '(dashboard :type git :flavor melpa 
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-export-creator-string "\\tiny{Emacs 28.0.50 (Org mode 9.4.4)}")
  ...
  (setq org-blank-before-new-entry (quote ((heading . t)
                                           (plain-list-item . nil))))
  (setq org-export-default-language "da")
  (setq org-export-backends '(ascii beamer bibtex html icalendar koma-letter 
                              latex md odt org publish rss texinfo))
...
;;            org-agenda-files "~/Dropbox/org/org-files"
            org-todo-keywords
            '((sequence "TODO(t)" "DOING(i)" "SOMEDAY(s)" "PHONE(p!)" 
                        "MØDE(m!)" "|" "DONE(d!)" "CANCEL(c@)"))
...
(use-package org-bullets
  ...
  :init (setq org-bullets-bullet-list '("⭐️" "🌀" "💢" "⚾️" "🔵" "🔴" "🔘")))
...
#+begin_src emacs-lisp :tangle no
(require 'pretty-symbols)
(straight-use-package '(org-superstar :type git :flavor melpa :host github 
...
(setq org-superstar-headline-bullet-list '("⭐️" "🌀" "💢" "⚾️" "🔵" "🔴" "🔘"))
...
(if nec/measure-time (nec/timer "org superstar "))
#+end_src
*** refiling Setup
:PROPERTIES:
...
#+begin_src emacs-lisp
        ...
        (setq org-capture-templates
          (doct '(("Aftale"
             ...
             :template ("* TODO %?                     %^G:AFTALE:"
                   ...
                   ("Telefon-Møde" 
                     :keys "k"
                     :file "~/Dropbox/org/agenda/opgaver.org"
                     :children
                       (:group "Tid"
                         :clock-in t
                         :children 
  ...
  (if nec/measure-time (nec/timer "doct and org-capture "))
  #+end_src
*** org agenda
  :PROPERTIES:
  ...
  :END:
  #+begin_src emacs-lisp 
    (setq diary-file "~/Dropbox/org/agenda/diary.org")
    (setq org-agenda-files '("~/Dropbox/org/agenda/organizer.org" 
                             "~/Dropbox/org/agenda/contacts.org" 
                             "~/Dropbox/org/agenda/Notes.org" 
                             "~/Dropbox/org/agenda/opgaver.org"))
    ;;(require 'org-mac-iCal)
    ;;(org-mac-iCal)
    (straight-use-package '(org-agenda :type git :repo "https://code.orgmode.org/bzg/org-mode.git" 
                                       :local-repo "org/lisp"))    
...
#+begin_src emacs-lisp  :tangle yes
  ...
  (straight-use-package '(origami :type git :flavor melpa :host github 
   ...
   (use-package org-super-agenda
      ...
      (setq org-super-agenda-groups
                '((:name "⏰ Calendar" :time-grid t)
                  ...
                  (:name "📌 Opgave" :category "OPGAVE")
                  (:name "📌 Opgave" :tag "OPGAVE")
...
*** org roam
:PROPERTIES:
:ID:       D931D321-FC5E-4F6A-9411-EDE1B78E766E
:END:
#+begin_src emacs-lisp :tangle yes
  ;;; jnf-org-roam.el --- Summary
  ;;; Commentary:
  ;;  From: https://github.com/jeremyf/dotzshrc/
...
(require 'org-backlink)  
  ...
  (defvar nec/open-roam-buffer-on-find-file t
  ...
  (use-package org-roam
    ...
    :bind (:map org-roam-mode-map
                (("C-c r l" . org-roam)
                 ("C-c r s" . org-roam-switch-to-buffer)
                ...
                (("C-c r c" . org-roam-capture))
                (("C-c r x" . org-roam-jump-to-index)))
    :config (setq org-roam-verbose nil
  ...
  (straight-use-package '(ox-hugo :type git :flavor melpa :host github :repo "kaushalmodi/ox-hugo"))
  (global-set-key (kbd "C-c r r") 'org-backlink-mode-refresh-rate)
  (global-set-key (kbd "C-c r s") 'org-backlink-mode-refresh-rate)  
  (global-set-key (kbd "C-c r m") 'org-backlink-mode)  

(setq org-roam-graph-viewer nil)
;;(setq org-roam-graph-viewer 'org-graph-view)
(setq org-roam-graph-executable "/usr/local/bin/dot")
;;(setq org-roam-graph-viewer #'eww-open-file)
;;(setq org-roam-graph-viewer "/usr/local/bin/firefox")
  (use-package org-roam-protocol
    ...
    (setq org-roam-capture-templates
          '(("a" "andet" plain (function org-roam--capture-get-point)
             "%?"
             ...
             :unnarrowed t
             :immediate-finish t)
            ("4" "4eren" plain (function org-roam--capture-get-point)
             "%?"
             :file-name "4eren/%<%Y%m%d%H%M%S>-${slug}"
             :head "#+title: ${title}\n#+roam_tags:\n :HOME: \n* ${title}\n\n"
             :unnarrowed t)
            ("b" "bibliographic" plain (function org-roam--capture-get-point)
             "%?"
             :file-name "bibliography/%<%Y%m%d%H%M%S>-${slug}"
             :head "#+title: ${title}\n#+roam_tags:\n :HOME: \n* ${title}\n\n"
             :unnarrowed t)
            ("u" "roam unsorted" plain (function org-roam--capture-get-point)
             "%?"
             :file-name "%<%Y%m%d%H%M%S>-${slug}"
             :head "#+title: ${title}\n#+roam_tags:\n* ${title}\n\n"
             :unnarrowed t)
            ("e" "emacs" plain (function org-roam--capture-get-point)
             "%?"
             :file-name "emacs/%<%Y%m%d%H%M%S>-${slug}"
             :head "#+title: ${title}\n#+roam_tags:\n :HOME: \n* ${title}\n\n"
             :unnarrowed t)

  ;;          ("d" "Project > Diversity Equity Incluson (DEI)" plain (function org-roam--capture-get-point)
  ;;           "%?"
  ;;           :file-name "projects/diversity-equity-inclusion/%<%Y%m%d%H%M%S>---${slug}"
  ;;           :head  "#+title: ${title}\n#+roam_tags:\n* ${title}\n\n"
  ;;           :unnarrowed t
  ;;           :immediate-finish t)
            ("i" "iniziu" plain (function org-roam--capture-get-point)
             "%?"
             :file-name "iniziu/%<%Y%m%d%H%M%S>-${slug}"
             :head  "#+title: ${title}\n#+roam_tags:\n :HOME: \n* ${title}\n\n"
             :unnarrowed t
             :immediate-finish t)
            ("s" "seniorråd" plain (function org-roam--capture-get-point)
             "%?"
             :file-name "seniorraad/%<%Y%m%d%H%M%S>---${slug}"
             :head  "#+title: ${title}\n#+roam_tags:\n :HOME: \n* ${title}\n\n"
             :unnarrowed t
             :immediate-finish t)
  ;;          ("o" "andre" plain (function org-roam--capture-get-point)
  ;;           "%?"
  ;;           :file-name "/roam/andre/%<%Y%m%d%H%M%S>---${slug}"
  ;;           :head  "#+title: ${title}\n#+roam_tags:\n* ${title}\n\n"
  ;;           :unnarrowed t
  ;;           :immediate-finish t)
            ("n" "niels" plain (function org-roam--capture-get-point)
             "%?"
             :file-name "niels/%<%Y%m%d%H%M%S>-${slug}"
             :head  "#+title: ${title}\n#+roam_tags:\n :HOME: \n* ${title}\n\n"
             :unnarrowed t
             :immediate-finish t)))
  (if nec/measure-time (nec/timer "org-roam-capture "))
  ;;;;;;;;;;
  (setq org-roam-graph-exclude-matcher '("archive" "PRIVAT" "blog" "books" "agenda"
                                         "include" "elfeed" "Documentation" "auto" "elfeed"
...
;;;;;;;;;;;
  (use-package org-roam-server
    ...
    (setq org-roam-server-host "127.0.0.1"
          ...
          org-roam-server-served-file-extensions '("pdf" "mp4" "svg" "ogv")    
  ...
  (defun jnf/make-filepath (title now &optional zone)
    ...
    (f-join
     ...
     "roam"
     (concat
      (format-time-string "%Y%m%d%H%M%S-" now (or zone (current-time-zone)))
...
(use-package org-super-links
  :straight (org-super-links :host github :repo "toshism/org-super-links" 
             :files ("*" (:exclude ".git")) :commit "0.3")
  :bind (("C-c s s" . org-super-links-link)
         ("C-c s l" . org-super-links-store-link)
         ("C-c s C-l" . org-store-super-links-insert-link)
         ("C-c s d" . org-super-links-quick-insert-drawer-link)
         ("C-c s i" . org-super-links-quick-insert-inline-link)
         ("C-c s r" . org-backlink-mode-refresh-rate)
         ("C-c s C-d" . org-super-links-delete-link))
  :config
  (setq org-super-links-backlink-into-drawer "~/Dropbox/org/roam/Index.org"
        org-super-links-related-into-drawer  "~/Dropbox/org/roam/Index.org" 
        org-super-links-link-prefix 'org-super-links-link-prefix-timestamp))
#+end_src
*** deft
#+begin_src emacs-lisp  :tangle yes
(straight-use-package '(deft :type git :flavor melpa 
                             :host github :repo "jrblevin/deft"))
...
(setq
 ...
 bibtex-completion-bibliography "~/Dropbox/org/roam/zotLib.bib"
...
;;  ":DOI: ${doi}\n"
  ":URL: ${url}\n"
  ":END:\n\n"))
 (if nec/measure-time (nec/timer "deft "))
#+end_src
*** zetteldeft
#+begin_src emacs-lisp :tangle no
...
(setq deft-recursive t)
;; Search the deft directory recursively, to include subdirectories.n 
(straight-use-package '(zetteldeft :type git :flavor melpa 
                        :files (:defaults "zetteldeft.org"
                        "zetteldeft-pkg.el") :host github :repo "EFLS/zetteldeft"))
(zetteldeft-set-classic-keybindings) 
(setq zetteldeft-home-id "Index.org")
(setq zetteldeft-id-format "%Y%m%d%H%M%S")
(setq zetteldeft-id-regex "20[0-9]\\{12\\}")
#+end_src 
I tend to write =org-mode= titles with =#+title:= (i.e.,
uncapitalized). Also other =org-mode= code at the beginning is written in
lower case.

In order to filter these from the deft summary, let's alter the
regular expression:
...
#+begin_src emacs-lisp :tangle no
  (setq deft-strip-summary-regexp
   (concat "\\("
           ...
           "\\)"))
(if nec/measure-time (nec/timer "Zettledeft "))
#+end_SRC
...
*** org ref
#+begin_src emacs-lisp  :tangle yes
(straight-use-package '(org-ref :type git :flavor melpa :files 
...
(setq bibtex-completion-bibliography "~/Dropbox/org/bibliography/references.bib"
      ...
      bibtex-completion-notes-path "~/Dropbox/org/bibliography/helm-bibtex-notes")
;; open pdf with system pdf viewer (works on mac)
(setq
         org-ref-completion-library 'org-ref-ivy-cite
         org-ref-get-pdf-filename-function 'org-ref-get-pdf-filename-helm-bibtex
;;         org-ref-default-bibliography (list "/home/haozeke/GDrive/zotLib.bib")
         org-ref-bibliography-notes "~/Dropbox/org/roam/bibnotes.org"
         org-ref-note-title-format "* TODO %y - %t\n :PROPERTIES:\n  :Custom_ID: %k\n  :NOTER_DOCUMENT: %F\n :ROAM_KEY: cite:%k\n  :AUTHOR: %9a\n  :JOURNAL: %j\n  :YEAR: %y\n  :VOLUME: %v\n  :PAGES: %p\n  :DOI: %D\n  :URL: %U\n :END:\n\n"
         org-ref-notes-directory "~/Dropbox/org/roam/Notes/"
...
(straight-use-package '(ebib :type git :flavor melpa :host github :repo "joostkremers/ebib"))
(global-set-key "\C-ce" 'ebib)
(require 'org-ebib)
(setq ebib-default-directory "~/Dropbox/org/bibliography/")
(setq ebib-file-search-dirs '("~/Dropbox/org/bibliography/"))
(setq ebib-filters-default-file "~/.emacs.d/private/ebib-filters")
(setq ebib-filters-default-file "~/.emacs.d/private/ebib-filters")
(setq ebib-extra-fields
   '((biblatex "crossref" "annotation" "abstract" "keywords" "file" "timestamp" "isbn")
     (BibTeX "crossref" "annote" "abstract" "keywords" "file" "timestamp" "url" "doi")))
(setq ebib-bibtex-dialect "biblatex")
(setq ebib-hide-cursor nil)
(setq ebib-uniquify-keys t)
(setq ebib-use-timestamp t)
(setq ebib-default-dir "/Users/niels/Dropbox/org/bibliography/")
#+end_src
*** org contact
:PROPERTIES:
...
*** org tree slide 
slide
Do presentation directly from Emacs
#+begin_src emacs-lisp :tangle yes

 (straight-use-package '(moom :type git :flavor melpa :host github :repo "takaxp/moom"))
 (with-eval-after-load "moom"
  ;; add settings here ...
  (moom-mode 1))

(straight-use-package '(org-tree-slide :type git :flavor melpa 
                           :host github :repo "takaxp/org-tree-slide"))
...
(setq org-mac-iCal-calendar-names '("niels" "lilian" "Søndergaard"))
...
#+END_SRC
*** væk mig
A package to wake me up, whenever I shall do something
#+begin_src emacs-lisp  :tangle yes
(require 'alert)
(require 'org-timed-alerts)
  ...
  (setq org-timed-alert-final-alert-string "Det er %alert-time\n\n%todo %headline")
  (setq org-timed-alert-warning-string (concat "%todo %headline\n at %alert-time\n "
                                               "klokken er %current-time\n "
                                               "*Dette er din %warning-time minuts advarsel*"))
...
:END:
#+begin_src emacs-lisp  :tangle yes
      (straight-use-package '(auctex :type git :host github :repo "emacs-straight/auctex" 
                                     :files ("*" (:exclude ".git"))))
...
#+begin_src emacs-lisp
  ...
  (if nec/measure-time (nec/timer "initiate appt/diary "))
    ...
    (if nec/measure-time (nec/timer "Writing calendar "))
      ...
      (if nec/measure-time (nec/timer "Calendar "))
#+end_src
...
*** gnuplot
#+begin_src emacs-lisp
(straight-use-package '(gnuplot :type git :flavor melpa 
      :files ("gnuplot.el" "gnuplot-gui.el" "gnuplot-context.el" "gnuplot-pkg.el") 
      :host github :repo "emacsorphanage/gnuplot"))
...
#+begin_src emacs-lisp
;; docker
(straight-use-package '(docker :type git :flavor melpa :host github :repo "Silex/docker.el"))
...
(setenv "DOCKER_CERT_PATH" "/Users/niels/.docker/machine/machines/default")
...
(if nec/measure-time (nec/timer "Tæl ord "))

(straight-use-package
 '(company-wordfreq :type git :host github :repo "johannes-mueller/company-wordfreq.el"))
(setq company-wordfreq-path
      (concat (file-name-as-directory user-emacs-directory) "private/wordfreq"))
(add-hook 'text-mode-hook (lambda ()
                            (setq-local company-backends '(company-wordfreq))
                            (setq-local company-transformers nil)))
(if nec/measure-time (nec/timer "Tæl alle ord "))
...
#+begin_src emacs-lisp
  ...
  (setq gnus-default-nntp-server "http://www.dr.dk/nyheder/service/feeds/viden")
  ...
  (setq gnus-select-method '(nntp "https://www.dr.dk/nyheder/service/feeds/viden"))
      (add-to-list 'gnus-secondary-select-methods '(nnml "mail.algon.dk"))
  (setq gnus-select-method
        '(nnimap "algon"
                 (nnimap-address "mail.algon.dk")
                 (nnimap-stream plain)
                 (nnimap-user "nisoni")
                 (nnimap-password "baldur2780")))
...
#+end_src
;; 
;; Elfeed allows to interactively subscribe to a feed (defaulting to what is in the
;; clipboard. Managing elfeed-feeds as a variable is kinda clumsy, although very
;; flexible. There is a middle ground which fits me even better. At the cost of the
;; bare-bone flexibilith of having the feeds directly in lisp, I'm using an orgmode
;; file to record the feeds i want.
 
#+begin_src emacs-lisp
(setq rmh-elfeed-org-files (list "~/Dropbox/org/elfeed/elfeed.org"))  
(straight-use-package '(elfeed-org :type git :flavor melpa 
                        :host github :repo "remyhonig/elfeed-org"))
...
*** ledger-mode
Ledger mode for MacBook Air
#+begin_src emacs-lisp
(straight-use-package '(ledger-mode :type git :flavor melpa :files (:defaults "ledger-test.el" "ledger-mode-pkg.el") :host github :repo "ledger/ledger-mode"))
(straight-use-package '(company-ledger :type git :flavor melpa :host github :repo "debanjum/company-ledger"))
(straight-use-package '(flycheck-ledger :type git :flavor melpa :host github :repo "purcell/flycheck-ledger"))
(straight-use-package '(dklrt :type git :flavor melpa :files ("*.el" "*.texi" "dir" "*.py" "dklrt-pkg.el") :host github :repo "davidkeegan/dklrt"))
(straight-use-package '(ledger-import :type git :flavor melpa :host github :repo "DamienCassou/ledger-import"))
(if nec/measure-time (nec/timer "ledger "))
#+end_src
* Languages
*** Yasnippets
...
#+begin_src emacs-lisp  :tangle no
(straight-use-package '(yasnippet :type git :flavor melpa 
                                  :files ("yasnippet.el" "snippets" "yasnippet-pkg.el") 
                                  :host github :repo "joaotavora/yasnippet"))
(add-to-list 'load-path "~/.emacs.d/snippets/")
(require 'yasnippet)
(yas-global-mode 1)
(if nec/measure-time (nec/timer "YASnippets "))
#+end_src
*** Projectile
#+begin_src emacs-lisp :tangle no
(straight-use-package '(projectile :type git :flavor melpa 
               :files ("projectile.el" "projectile-pkg.el") 
               :host github :repo "bbatsov/projectile"))
(setq projectile-project-search-path '("~/Projects/"))
(projectile-mode +1)
(straight-use-package '(counsel-projectile :type git :flavor melpa 
               :host github :repo "ericdanan/counsel-projectile"))
(general-define-key
...
:END:
We want to give all nye (and old) files a header
#+begin_src emacs-lisp :tangle yes
(require 'file-template)
;; we want headers on org and org-roam files too
;;(add-to-list 'file-template-mapping-alist ("\\.org$" . "template.org"))
;;(add-to-list 'file-template-mapping-alist ("20*.org$" . "org-template.sh"))
...
(if nec/measure-time (nec/timer "Header on files  "))
#+end_src
*** Python
:PROPERTIES:
:CUSTOM_ID: Python
:END:
#+begin_src emacs-lisp :tangle no
(straight-use-package '(python :type git :host github 
...
(use-package elpy
  ...
  ;;
  (straight-use-package '(ein :type git :flavor melpa 
          :files ("lisp/*" (:exclude "lisp/zeroein.el") "ein-pkg.el") 
          :host github :repo "millejoh/emacs-ipython-notebook"))
  (if nec/measure-time (nec/timer "ipython  "))
#+end_src
*** Rust
:PROPERTIES:
:CUSTOM_ID: Rust
...
#+begin_src emacs-lisp
;;; start the watcher (see straight.el documentation)
